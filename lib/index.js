...
function reset() {
	...
	function confirmation(cb) {
		process.stdout.write('Are you sure you want to delete ' + config.db.database +  '? [y/n] ');
		process.stdin.once('data', function(data) {
			var response = data.toString().trim();
			if(response == 'y') {
				cb();
			} else {
				cb(new Error('Did not confirm reset.'));
			}
		});
	}

	function finalConfirmation(cb) {
		process.stdout.write('To confirm, type: "DELETE ALL DATA FOR: ' + config.db.database +  '" ');
		process.stdin.once('data', function(data) {
			var response = data.toString().trim();
			if(response == 'DELETE ALL DATA FOR: ' + config.db.database) {
				cb();
			} else {
				cb(new Error('Did not confirm reset.'));
			}
	...
	function wipe(cb) {
		var i;
		var tables = [];
		var query = connection.query('SHOW TABLES', function(err, rows, fields) {
			if(err || rows.length == 0) {
				cb(new Error('There were no tables in the database.'));
			} else {
				for(i = 0; i < rows.length; i++) {
					tables.push(rows[i]['Tables_in_' + config.db.database]);
				}

				async.eachSeries(tables, function(item, cb2) {
					var query = connection.query('DROP TABLE ??', [item], function(err, rows, fields) {
						cb2(err);
					});
				}, cb);
			}
